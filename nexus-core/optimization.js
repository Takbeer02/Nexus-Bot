...
class Optimization {
  ...
  static initErrorTracking() {
    try {
      ...
      // Add process-level error tracking
   ...
   */
  static startMemoryMonitor(interval = 60000) {
    try {
      // Start periodic memory checks
      setInterval(() => {
        const used = process.memoryUsage();
        console.log(`Memory Usage: RSS=${used.rss / 1024 / 1024}MB, Heap=${used.heapUsed / 1024 / 1024}MB`);

        if (global.gc) {
          console.log('Running garbage collection...');
          global.gc();
        } else {
          // Only log this warning once per process
          if (!this.gcWarningShown) {
            console.warn('Garbage collection is not exposed. Run Node.js with --expose-gc.');
            this.gcWarningShown = true;
          }
        }
      }, interval);
