...
class AutoRecovery {
  ...
  static initErrorTracking() {
    ...
    const autoRestartConfig = config.systemConfig?.system?.autoRestart || {};
  ...
  static async checkRestartMarker(api) {
    ...
    try {
      if (fs.existsSync(markerPath)) {
        ...
        if (api && global.config?.botConfig?.admins?.length > 0) {
          await api.sendMessage(
            ...
            global.config.botConfig.admins[0]
  ...
  static createErrorRestartMarker(reason, threadID) {
    try {
      ...
      fs.writeFileSync(markerPath, JSON.stringify({
        time: Date.now(),
        threadID: threadID || (global.config?.botConfig?.admins?.[0] ? global.config.botConfig.admins[0] : null),
        userID: 'SYSTEM',
