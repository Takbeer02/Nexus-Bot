...
module.exports = function(config) {
  ...
  async function initialize() {
    ...
    try {
      ...
      if (config.botConfig.owner) {
        userRoleCache.set(config.botConfig.owner, 3);
        
        // Ensure owner is in database
        await new Promise((resolve, reject) => {
          db.run('INSERT OR REPLACE INTO users (id, role) VALUES (?, 3)', [config.botConfig.owner], (err) => {
            if (err) reject(err);
            else resolve();
          });
        });
      ...
      if (Array.isArray(config.botConfig.admins)) {
        for (const adminId of config.botConfig.admins) {
  ...
  async function getUserRole(userId) {
    ...
    if (userId === config.botConfig.owner) {
      return 3;
    }
    ...
    if (Array.isArray(config.botConfig.admins) && config.botConfig.admins.includes(userId)) {
  ...
  async function getAdminUsers() {
    try {
      ...
      const admins = await new Promise((resolve, reject) => {
        db.all('SELECT id, role FROM users WHERE role >= 2', (err, rows) => {
          if (err) reject(err);
          else resolve(rows || []);
        });
      });
      ...
      // Add any admins from config that might not be in database
      const config = global.config || {};
      if (config.botConfig.admins && Array.isArray(config.botConfig.admins)) {
        for (const adminId of config.botConfig.admins) {
          if (!admins.some(admin => admin.id === adminId)) {
            admins.push({ id: adminId, role: 2 });
          }
        }
      }
      
      // Add owner if not already included
      if (config.botConfig.owner && !admins.some(admin => admin.id === config.botConfig.owner)) {
        admins.push({ id: config.botConfig.owner, role: 3 });
      }
